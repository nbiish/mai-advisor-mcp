...
# Note: GrantResearchAgent available for MCP server mode (requires Tavily API)
...
""",
        "crash_course": f"""# The "Make It Happen" Crash Course
**Generated:** {timestamp}
**Topic:** {topic}
**Location:** {location or "Not specified"}
...
"""
    ...
    # Check for crash course plan
    crash_course_plan = next((p for p in expert_plans if p["expert"] == "crash_course"), None)
    
    if crash_course_plan:
        final_plan += f"""
...
def run_complete_workflow(topic: str, location: str = "", api_key: str = ""):
    """
    ...
    Args:
        topic: Grant topic/focus area
        location: Geographic location (optional)
        api_key: Anthropic API key (required for AI features)
    """
    # Validate inputs
    ...
    # Track generated files for session export
    generated_files = [dorks_filepath]
    ...
    status += f"🔑 Using OpenRouter API key: {api_key[:15]}...{api_key[-4:]}\n"
    status += f"🤖 Model: Google Gemini 2.5 Flash (Preview)\n\n"
    
    expert_names = ["financial", "grant", "research", "crash_course"]
    expert_plans = []
    ...
    generated_files.append(final_filepath)
    ...
    generated_files.append(ai_todo_filepath)
...
"""
    ...
    return (
        ...
        generated_files # Return list of generated files
    )
...
def export_current_session(file_paths):
    """Create a zip file of generated data from the current session."""
    if not file_paths:
        return None, "⚠️ No files generated in this session yet. Run the workflow first."
    
    zip_path = output_manager.create_session_export_zip(file_paths)
    return zip_path, f"✅ Export ready: {Path(zip_path).name}"
...
with gr.Blocks(title="MAI Advisor - Grant Planning System", theme=gr.themes.Soft()) as app:
    ...
    # State to track files generated in the current session
    session_files = gr.State([])
    ...
    with gr.Tab("🚀 Run Workflow"):
        ...
        gr.Markdown("""
        ⚠️ **API Key Required:** This application requires an OpenRouter API key to generate AI-powered strategic frameworks using Google Gemini 2.5 Flash.
        ...
        api_key_input = gr.Textbox(
            ...
            type="password",
            lines=1,
            info="Required. Your key is used only for this session and never stored. Get one at openrouter.ai/keys"
        )
        ...
        with gr.Row():
            export_btn = gr.Button("📦 Export Session Data (ZIP)", variant="secondary")
        ...
        run_btn.click(
            ...
            inputs=[topic_input, location_input, api_key_input],
            outputs=[workflow_status, dorks_output, financial_output, grant_output, research_output, final_output, ai_todo_output, session_files]
        )
